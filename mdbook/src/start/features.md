...
The feature definitions and their rationale live in the crate's `realtime_ndt_scan_matcher/Cargo.toml`; the engine's
interior-mutability matrix is documented at the top of `realtime_ndt_scan_matcher/src/engine.rs`; the crate-level overview
is the `//!` doc in `realtime_ndt_scan_matcher/src/lib.rs`.
