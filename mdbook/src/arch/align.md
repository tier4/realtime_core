...
`ndt::align` (`realtime_ndt_scan_matcher/src/ndt.rs`) is the RT-critical alignment kernel — the Rust equivalent of the
...
(`realtime_ndt_scan_matcher/tests/zero_alloc.rs`) and audited by a real-time review of each engine/align patch.
...
> Source: the `align` doc + WCET contract in `realtime_ndt_scan_matcher/src/ndt.rs` (≈ lines 317, 617); `realtime_ndt_scan_matcher/src/derivatives.rs`;
> `realtime_ndt_scan_matcher/src/voxel_grid.rs`; `realtime_ndt_scan_matcher/tests/zero_alloc.rs`; `realtime_ndt_scan_matcher/examples/wcet_frame.rs`.
