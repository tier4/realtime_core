...
`NdtEngine` (`realtime_ndt_scan_matcher/src/engine.rs`) is the persistent handle that wraps the target map + params over a
...
> Source: the module doc at the top of `realtime_ndt_scan_matcher/src/engine.rs`; `realtime_ndt_scan_matcher/tests/concurrency.rs`.
