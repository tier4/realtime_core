...
The per-frame WCET micro-benchmark `realtime_ndt_scan_matcher/examples/wcet_frame.rs` measures the tail directly; current
numbers are in Benchmarking. A
`rust-realtime-review` accompanies each engine/align patch.

> Source: `realtime_ndt_scan_matcher/src/ndt.rs` (the `align` WCET contract), `realtime_ndt_scan_matcher/examples/wcet_frame.rs`.
