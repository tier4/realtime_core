...
`realtime_ndt_scan_matcher/tests/zero_alloc.rs` asserts zero allocations per frame after warmup (an allocator hook counts
...
> Source: `realtime_ndt_scan_matcher/tests/zero_alloc.rs`; `realtime_ndt_scan_matcher/src/ndt.rs` (`AlignWorkspace`), `realtime_ndt_scan_matcher/src/engine.rs` (`MatchScratch`).
