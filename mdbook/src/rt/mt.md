...
otherwise `awkernel_sync` does not compile. `realtime_ndt_scan_matcher/examples/threads_ndt.rs` exercises shared-engine usage;
the host-testable lint gate is `--features mt,awkernel_sync/std` (see
Build and test).

> Source: `realtime_ndt_scan_matcher/src/engine.rs` (`mt` cfg paths), `realtime_ndt_scan_matcher/Cargo.toml` (`mt` feature), `realtime_ndt_scan_matcher/examples/threads_ndt.rs`.
