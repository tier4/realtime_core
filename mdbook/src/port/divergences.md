...
- **NDT `h_ang` "d1" sign bug — fixed upstream (PR #1217), port followed.**
  ...
  the then-buggy C++; when the upstream fix landed the port followed it (`realtime_ndt_scan_matcher/src/derivatives.rs`, now
  ...
  (`update_derivatives_gradient_and_hessian_match_finite_difference` in `realtime_ndt_scan_matcher/src/derivatives.rs`,
  `compute_derivatives_matches_finite_difference` in `realtime_ndt_scan_matcher/src/ndt.rs`) cover all six rows and pin the
...
- **Step-length clamp panic — a port bug (not upstream), fixed 2026-07-10.**
  ...
  mirrors the exact min-then-max order (`realtime_ndt_scan_matcher/src/ndt.rs`; Rust `f64::min`/`max` also match
...
- **Non-finite result pose → non-converged** (`realtime_ndt_scan_matcher/src/engine.rs`, `run_align`): the
...
- **Softmax temperature guard** (`realtime_ndt_scan_matcher/src/covariance.rs`, `calc_weight_vec`): non-positive or
...
> Source: `realtime_ndt_scan_matcher/src/derivatives.rs` (the `h_ang` table + FD tests); upstream issue/PR links.
