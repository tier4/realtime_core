...
This book covers the `realtime_core` workspace crates — the engine crate
`realtime_ndt_scan_matcher` and the `realtime_localization_util` / `realtime_kdtree` /
`realtime_port_instrument` crates it builds on. The FFI boundary, the
...
| **An upstream / PR reviewer** | [Numeric parity](port/numeric-parity.md) and [Divergences](port/divergences.md) | [Differential testing](port/differential.md), [Trace verification](port_instrument/trace-verification.md) |
...
- **Paths** like `realtime_ndt_scan_matcher/src/engine.rs` are relative to **the workspace root**
  (`realtime_core/`).
